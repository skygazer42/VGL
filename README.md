...
- **Mini-batch neighbor sampling** — `NodeNeighborSampler`, `LinkNeighborSampler`, and `TemporalNeighborSampler` provide PyG/DGL-style local subgraph training for homogeneous, heterogeneous, and temporal node/link workloads, including relation-aware temporal event sampling for typed heterogeneous graphs, with opt-in plan-backed node/edge feature materialization for sampled node, link, and temporal batches. VGL also now exposes `to_block(...)` plus a lightweight `Block` container for relation-local message-flow rewrites when one batch should be viewed explicitly as source/destination frontiers instead of one sampled subgraph. For homogeneous node workloads, `NodeNeighborSampler(..., output_blocks=True)` materializes `NodeBatch.blocks` in outer-to-inner order while keeping `batch.graph` and `batch.seed_index` unchanged; this now covers both local full-graph sampling and stitched homogeneous shard-local sampling through a coordinator-backed feature source. For homogeneous link workloads, `LinkNeighborSampler(..., output_blocks=True)` now does the same without changing `batch.graph`, `src_index`, `dst_index`, or `labels`; this now covers both local full-graph sampling and stitched homogeneous shard-local sampling through a coordinator-backed feature source, and when seed-edge exclusion is active those excluded supervision edges are removed from the block message-passing view as well. Heterogeneous stitched link block output remains unsupported in this batch. On shard-local graphs, coordinator-backed node and link sampling can now stitch cross-partition frontier nodes and edges into one sampled subgraph for homogeneous workloads and non-temporal heterogeneous node/link workloads, while `TemporalNeighborSampler` can stitch earlier cross-partition history for both homogeneous and typed heterogeneous temporal workloads.
- **Foundation layers for scale** — `vgl.sparse`, `vgl.storage`, `vgl.ops`, `vgl.data`, and `vgl.distributed` provide sparse adjacency views, storage-backed graphs, graph transforms including line graphs, inbound/outbound frontier subgraphs, relation-local `to_block(...)` message-flow rewrites, metapath reachability, relation-local hetero subgraphs/compaction, dataset catalogs / lazy on-disk formats, and local partition primitives plus typed node / edge routing, relation-scoped edge feature fetches, and partition-scoped local, boundary, and incident graph queries while keeping `Graph`, `Loader`, and `Trainer` as the public entry points.
...
| `vgl.ops` | structure transforms, frontier subgraphs, relation-local `to_block`, line graphs, random walks, metapath random walks, metapath reachability, homo/hetero relation-local subgraph extraction, k-hop expansion, compaction |
...
- `vgl.ops` centralizes reusable graph transforms such as self-loop rewrites, bidirection conversion, non-mutating `reverse(...)`, relation-local `to_block(...)` message-flow rewrites, line-graph construction, uniform `random_walk(...)`, typed `metapath_random_walk(...)`, metapath reachability, induced subgraphs, inbound/outbound frontier subgraphs, relation-local hetero subgraphs, relation-local hetero k-hop expansion, compaction, and edge-query helpers through `find_edges(...)`, `edge_ids(...)`, and `has_edges_between(...)`. `in_subgraph(...)` and `out_subgraph(...)` preserve the original node space while filtering edges by destination or source frontier and always expose selected `e_id`, which makes them complementary to `node_subgraph(...)` rather than a replacement for compaction; the new query helpers resolve against that stable public edge-id space whenever it is present. `reverse(...)` keeps edge order stable while swapping source and destination endpoints, preserves `e_id`, and retains node-count context on featureless storage-backed graphs. `to_block(...)` returns a lightweight `Block` container carrying a compacted relation-local bipartite graph plus `src_n_id` / `dst_n_id` / `e_id` metadata, while `Graph.to_block(...)` exposes the same transform as a convenience bridge. Homogeneous `NodeNeighborSampler(..., output_blocks=True)` and `LinkNeighborSampler(..., output_blocks=True)` materialize the same `Block` container per hop from sampled neighborhoods so those ids stay aligned with sampled `n_id` / `e_id` instead of full-graph neighborhoods; for link sampling, the blocks are derived from the sampled message-passing view so excluded supervision edges stay out of the block layers. For bipartite heterogeneous relations, `khop_nodes(...)` expects seeds keyed by node type and returns per-type node ids when an `edge_type` is selected, `random_walk(...)` can target one selected relation when it composes with itself, and `metapath_reachable_graph(...)` / `metapath_random_walk(...)` compose a typed metapath into reachable endpoints or sampled path traces.
...
Each node-sampling dataset item can carry one seed or a rank-1 seed collection in `metadata["seed"]`. `NodeNeighborSampler` will sample one union subgraph for that item and materialize one flat `NodeBatch.seed_index` entry per requested seed. For heterogeneous graphs, keep passing `metadata["node_type"]` for the supervised node type. For homogeneous node sampling, `NodeNeighborSampler(..., output_blocks=True)` also exposes `NodeBatch.blocks` in outer-to-inner order without changing `batch.graph` or `batch.seed_index`; this now works for local full-graph sampling and stitched homogeneous shard-local sampling when the loader executes against a coordinator-backed feature source. Heterogeneous stitched node sampling still returns sampled subgraphs only in this batch.
...
On homogeneous graphs, `LinkNeighborSampler(..., output_blocks=True)` also exposes `LinkPredictionBatch.blocks` in outer-to-inner order while keeping `batch.graph`, `batch.src_index`, `batch.dst_index`, and `batch.labels` unchanged. This now covers local full-graph sampling plus stitched homogeneous shard-local sampling through a coordinator-backed feature source. Those blocks are derived from the sampled message-passing graph rather than a raw supervision view, so positive edges hidden by `exclude_seed_edge` / `exclude_seed_edges` stay excluded from block message passing as well. Heterogeneous stitched link sampling still return sampled subgraphs only in this batch.
