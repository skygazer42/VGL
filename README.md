...
Each node-sampling dataset item can carry one seed or a rank-1 seed collection in `metadata["seed"]`. `NodeNeighborSampler` will sample one union subgraph for that item and materialize one flat `NodeBatch.seed_index` entry per requested seed. For heterogeneous graphs, keep passing `metadata["node_type"]` for the supervised node type. When the source graph is local and homogeneous, `NodeNeighborSampler(..., output_blocks=True)` also exposes `NodeBatch.blocks` in outer-to-inner order without changing `batch.graph` or `batch.seed_index`; heterogeneous and coordinator-stitched node sampling still return sampled subgraphs only in this batch.
