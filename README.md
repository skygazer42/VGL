...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.1.4-8b5cf6?style=flat-square" alt="Version 0.1.4"/>
...
- **PyG, DGL, NetworkX, edge-list, and CSV-table interoperability** — bidirectional adapters for PyG data, homogeneous NetworkX graphs, in-memory homogeneous edge lists, CSV edge-list files, paired node/edge CSV tables, plus homogeneous, heterogeneous, and temporal DGL graph round-trips that preserve canonical edge types and temporal `time_attr` metadata, along with dedicated single-relation `Block` and multi-relation `HeteroBlock` <-> DGL block conversion for message-flow frontiers.
...
| `vgl.sparse` | `SparseTensor`, COO/CSR/CSC conversion helpers, torch sparse interop, multi-value payload support, transpose/reduction utilities, multi-value SPMM/SDDMM, edge softmax, sparse graph ops |
...
| `vgl.ops` | structure transforms, adjacency/Laplacian sparse views, frontier subgraphs, relation-local `to_block`, multi-relation `to_hetero_block`, line graphs, random walks, metapath random walks, metapath reachability, homo/hetero relation-local subgraph extraction, k-hop expansion, compaction |
...
| `vgl.compat` | PyG, DGL, NetworkX, edge-list, and CSV-table interoperability helpers |
...
- `vgl.ops` centralizes reusable graph transforms plus graph queries. Alongside `find_edges(...)`, `edge_ids(...)`, and `has_edges_between(...)`, it now exposes `num_nodes(...)` / `num_edges(...)` with `number_of_*` aliases for graph cardinality, `all_edges(...)` for DGL-style full-edge enumeration, graph format-state helpers through `formats(...)` and `create_formats_(...)`, `adj(...)` for DGL-style weighted adjacency sparse views, `laplacian(...)` for square-relation Laplacian sparse views with `None`, `"rw"`, or `"sym"` normalization, `adj_external(...)` for torch / SciPy sparse adjacency export, `adj_tensors(...)` for raw COO/CSR/CSC adjacency tensor export, `inc(...)` for DGL-style incidence sparse views, `in_edges(...)`, `out_edges(...)`, `predecessors(...)`, and `successors(...)` for ordered one-hop adjacency, plus `in_degrees(...)` / `out_degrees(...)` for degree inspection. Count helpers return Python `int`, degree queries return `int` for one node and tensors for many or all nodes, and both continue to respect storage-backed declared node counts so isolated nodes surface as zero instead of disappearing. `formats()` reports graph-level sparse-format status, `create_formats_()` eagerly materializes all allowed formats, and `all_edges(order="eid")`, `adj(...)`, `laplacian(...)`, `adj_external(...)`, `adj_tensors(...)`, and `inc(...)` all follow the public `e_id` ordering model when present. `adj(..., eweight_name="weight")` uses edge features as sparse values, `laplacian(..., normalization="rw" | "sym")` builds row-degree-based normalized Laplacians on square relations while preserving declared storage-backed node space, `adj_external(...)` returns `torch.sparse_coo_tensor` by default, can emit native torch COO / CSR / CSC layouts through `torch_fmt=`, and still exports SciPy `coo` / `csr` matrices through `scipy_fmt=`, all with unit values. `adj_tensors("coo")` returns `(src, dst)` while `adj_tensors("csr")` / `adj_tensors("csc")` return compressed pointers, compressed coordinates, and aligned public edge ids. `inc(typestr="both")` drops self-loop nonzeros just like DGL. `in_subgraph(...)` / `out_subgraph(...)` keep node space intact, `to_simple(...)` collapses parallel edges in stable first-occurrence order and can attach multiplicity counts through `count_attr=`, `reverse(...)` swaps endpoints while preserving `e_id` and storage-backed node-count context, `to_block(...)` keeps relation-local message-flow rewrites and `src_n_id` / `dst_n_id` / `e_id` metadata aligned, and `to_hetero_block(...)` builds one multi-relation heterogeneous message-flow layer from per-type destination frontiers.
