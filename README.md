...
`LinkNeighborSampler(..., output_blocks=True)` also exposes `LinkPredictionBatch.blocks` in outer-to-inner order while keeping `batch.graph`, `batch.src_index`, `batch.dst_index`, and `batch.labels` unchanged. This now covers local and stitched homogeneous link sampling plus relation-local heterogeneous link sampling through a coordinator-backed feature source. Those blocks are derived from the sampled message-passing graph rather than a raw supervision view, so positive edges hidden by `exclude_seed_edge` / `exclude_seed_edges` stay excluded from block message passing as well. On heterogeneous graphs the block list stays scoped to the supervised `edge_type`, so block-bearing batches still require one relation schema per sampled batch item.
