...
On local homogeneous graphs, `LinkNeighborSampler(..., output_blocks=True)` also exposes `LinkPredictionBatch.blocks` in outer-to-inner order while keeping `batch.graph`, `batch.src_index`, `batch.dst_index`, and `batch.labels` unchanged. Those blocks are derived from the sampled message-passing graph rather than a raw supervision view, so positive edges hidden by `exclude_seed_edge` / `exclude_seed_edges` stay excluded from block message passing as well. Heterogeneous and coordinator-stitched link sampling still return sampled subgraphs only in this batch.
