...
This is what makes many-small-graph and sampled-subgraph inputs converge on the same batch contract. For local homogeneous node sampling, `SampleRecord` can now also carry per-hop `blocks` derived from the sampled subgraph.
