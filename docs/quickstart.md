...
Plan-backed feature fetch stages can also use the same routed source directly through `PlanExecutor.execute(..., feature_store=coordinator)` or `Loader(..., feature_store=coordinator)` when you want executor-driven feature access instead of direct store access. Those explicit arguments remain the highest-priority override; otherwise, storage-backed graphs can supply the same context through their retained `graph.feature_store`. When the source graph is a shard-local `shard.graph`, `NodeNeighborSampler` can now use coordinator incident-edge queries to stitch remote frontier nodes and edges into the sampled subgraph for homogeneous workloads and for non-temporal heterogeneous node workloads, while keeping node and edge tensors aligned through global `n_id` / `e_id`. Heterogeneous link sampling and temporal stitched sampling still use local structure plus routed feature fetch only.
