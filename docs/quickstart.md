...
- `vgl.ops` for reusable graph transforms, edge queries, `reverse(...)`, frontier subgraphs, relation-local `to_block(...)`, line graphs, random walks, metapath random walks, metapath reachability, homogeneous/heterogeneous relation-local subgraph extraction, relation-local k-hop expansion, and compaction
...
Use `graph.in_subgraph(nodes)` or `graph.out_subgraph(nodes)` when you want inbound or outbound frontier edges without compacting node ids. Those ops preserve the original node space, so they complement `node_subgraph(...)` instead of replacing it. Use `graph.find_edges(eids)`, `graph.edge_ids(u, v)`, and `graph.has_edges_between(u, v)` when you need stable edge queries; if a derived graph already exposes `edata["e_id"]`, those lookups continue to resolve in that public edge-id space. Use `graph.reverse(...)` when you want the same edge order with swapped source/destination endpoints while keeping storage-backed node-count context intact. For relation-local heterogeneous graph ops that really compact or expand one chosen relation, keep passing `edge_type=...` and provide bipartite `khop_nodes(...)` seeds as `{node_type: ids}` so the returned node ids stay partitioned by node type and can flow directly into `khop_subgraph(...)`. Use `graph.to_block(dst_nodes, edge_type=...)` when one selected relation should be rewritten into explicit source/destination frontiers, `graph.line_graph(...)` when you want one edge-centric homogeneous topology over a selected relation, `graph.random_walk(seeds, length=..., edge_type=...)` when you want uniform relation-local walk traces, and `graph.metapath_reachable_graph([...])` or `graph.metapath_random_walk(seeds, [...])` when you want typed heterogeneous metapath reachability or sampled path traces. For homogeneous node mini-batches, `NodeNeighborSampler(..., output_blocks=True)` keeps the existing sampled `NodeBatch.graph` and `NodeBatch.seed_index` contract and also adds `NodeBatch.blocks` in outer-to-inner order; this now works for local full-graph sampling and stitched homogeneous shard-local sampling through a coordinator-backed feature source. Homogeneous link mini-batches can now do the same through `LinkNeighborSampler(..., output_blocks=True)`, which adds `LinkPredictionBatch.blocks` without changing `batch.graph`, `src_index`, `dst_index`, or `labels`; this now works for local full-graph sampling and stitched homogeneous shard-local sampling through a coordinator-backed feature source, and when seed-edge exclusion is active those excluded supervision edges stay out of the block message-passing view. Heterogeneous stitched link block output still expose sampled subgraphs only in this batch.
...
When you want DGL-style message-flow blocks for a homogeneous node workload, switch the loader to `NodeNeighborSampler([15, 10], output_blocks=True)`. The resulting `NodeBatch` still exposes `batch.graph` and `batch.seed_index`, and now also exposes `batch.blocks[0]`, `batch.blocks[1]`, ... ordered from the outer frontier back to the seed frontier. This path now works for local full-graph sampling and stitched homogeneous shard-local sampling when the loader uses a coordinator-backed feature source. Each emitted `Block` can now also round-trip through DGL with `batch.blocks[0].to_dgl()` or `Block.from_dgl(dgl_block)` when the external block is single-relation.
...
For homogeneous link sampling, `LinkNeighborSampler(..., output_blocks=True)` keeps the existing `LinkPredictionBatch.graph`, `src_index`, `dst_index`, and `labels` contract and additionally materializes `LinkPredictionBatch.blocks` in outer-to-inner order. This now covers local full-graph sampling plus stitched homogeneous shard-local sampling through a coordinator-backed feature source. Those blocks are derived from the sampled message-passing graph, so positive seed edges removed by `exclude_seed_edge` / `exclude_seed_edges` stay absent from the block layers. Heterogeneous stitched link sampling still expose sampled subgraphs only in this batch.
