# Homogeneous Local, Steered, and Degree Convolution Pack Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add `EdgeConv`, `FeaStConv`, and `MFConv` as stable public homogeneous operators with focused tests, exports, integration coverage, and example updates.

**Architecture:** Extend the flat `vgl.nn.conv` operator surface with one local pairwise interaction layer, one feature-steered assignment layer, and one degree-conditioned projection layer. Reuse `_homo.py` helpers where possible, keep heterogeneous rejection consistent across operators, and avoid refactoring `MessagePassing`.
...
- `test_edge_conv_accepts_graph_input`
- `test_edge_conv_accepts_x_and_edge_index`
- `test_feast_conv_accepts_graph_input`
- `test_feast_conv_accepts_x_and_edge_index`
- `test_mfconv_accepts_graph_input`
- `test_mfconv_accepts_x_and_edge_index`
- hetero rejection coverage for `EdgeConv`, `FeaStConv`, and `MFConv`

Update `tests/test_package_exports.py` so the root package asserts:

- `EdgeConv.__name__ == "EdgeConv"`
- `FeaStConv.__name__ == "FeaStConv"`
- `MFConv.__name__ == "MFConv"`
...
### Task 2: Implement `EdgeConv`, `FeaStConv`, and `MFConv`

**Files:**
- Create: `vgl/nn/conv/edgeconv.py`
- Create: `vgl/nn/conv/feast.py`
- Create: `vgl/nn/conv/mfconv.py`
- Modify: `vgl/nn/conv/__init__.py`
- Modify: `vgl/nn/__init__.py`
- Modify: `vgl/__init__.py`

**Step 1: Implement `EdgeConv` minimally**

Create `vgl/nn/conv/edgeconv.py` with a layer that:

- accepts `in_channels`, `out_channels`, and `aggr`
- supports `forward(graph)` and `forward(x, edge_index)`
- uses homogeneous-only input coercion
- builds edge messages from `[x_dst, x_src - x_dst]`
- applies an internal MLP
- supports:
  - `max`
  - `sum`
  - `mean`

**Step 2: Implement `FeaStConv` minimally**

Create `vgl/nn/conv/feast.py` with a layer that:

- accepts `in_channels`, `out_channels`, and `heads`
- supports `forward(graph)` and `forward(x, edge_index)`
- computes feature-steered assignment logits from `x_src - x_dst`
- normalizes assignments with `edge_softmax(...)`
- projects source features into per-head value spaces
- aggregates head outputs and adds a root-node branch

**Step 3: Implement `MFConv` minimally**

Create `vgl/nn/conv/mfconv.py` with a layer that:

- accepts `in_channels`, `out_channels`, and `max_degree`
- supports `forward(graph)` and `forward(x, edge_index)`
- computes a neighborhood summary
- computes clipped destination in-degree buckets
- applies degree-conditioned neighborhood and root transforms
...
- the script prints result dictionaries that include `edgeconv`, `feast`, and `mfconv`
